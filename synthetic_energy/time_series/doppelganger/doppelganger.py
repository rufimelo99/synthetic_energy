...
from synthetic_energy.errors import DataError, InternalError, ParameterError, UserError
...
def _discrete_cols_to_int(
    ...
    if missing_discrete:
        logger.warning(
            "Columns not found in the DataFrame, you may want to ensure this is intended!",
            columns=missing_discrete,
        )
...
class DGAN:
    ...
    def train_numpy(
        ...
        logger.info(
            ...
            first_sequence_shape=features[0].shape,
        ...
        if feature_types is None:
            ...
            for i in range(features[0].shape[1]):
                try:
                    ...
                    # If we get a TypeError, it's likely because the column
                    # contains a timestamp. This should be pruned out before.
                    logger.warning(
                        "Feature column contains non-numeric data, please ensure all columns are numeric",
                        error=e,
                    )
                    raise UserError(
                        "Feature column contains non-numeric data, please ensure all columns are numeric"
        ...
        if not self.is_built:
            logger.info("Determining outputs metadata from input data")
            ...
            logger.info("Building DGAN networks")
            self._build(
                attribute_outputs,
                feature_outputs,
            )
        ...
        if continuous_features_ind:
            ...
            logger.info(
                "Checking for nans in the numeric columns",
                number_of_columns=len(continuous_features_ind),
            )
        ...
        logger.info("Creating encoded array of features")
        (
            internal_features,
        ...
        logger.info("Creating encoded array of attributes")
        ...
        logger.info(
            "Training...",
            internal_features_shape=internal_features.shape,
            dtype=internal_features.dtype,
            internal_additional_attributes_shape=internal_additional_attributes.shape,
            internal_attributes_shape=internal_attributes.shape,
        )
    ...
    def train_dataframe(
        ...
        if self.data_frame_converter is None:
            ...
            if df_style == DfStyle.WIDE:
                ...
                if example_id_column is None and attribute_columns is None:
                    logger.warning(
                        "The `example_id_column` was not provided, DGAN will autosplit dataset into sequences",
                        number_of_sequences=self.config.max_sequence_len,
                    )
        ...
        logger.info("Converting from DataFrame to numpy arrays")
        attributes, features = self.data_frame_converter.convert(df)

        logger.info("Calling train_numpy()")
        self.train_numpy(
    ...
    def create(
        ...
        for column_name in df.columns:
            if df[column_name].dtype == "O":
                logger.debug(
                    "Marking column as discrete because its type is string/object.",
...
def validation_check(
    ...
    if (~valid_examples).any():
        logger.warning(
            "There are examples that have too many nan values in numeric features, accounting for % of all examples. These invalid examples will be omitted from training.",  # noqa
            invalid_examples=sum(~valid_examples),
            num_examples={np.mean(~valid_examples) * 100},
        )
